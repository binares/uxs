...
class hitbtc(ExchangeSocket):
    ...
    def check_errors(self, r):
        ...
        e = r["error"]
        d = {
            x: self.api.safe_string(e, x, "")
            for x in ("message", "description", "code")
        }
    ...
    def on_orderbook(self, r):
        ...
        # in that case the sequence is different, and we ignore the update
            ...
            self.log(
                "reloading orderbook {} due to unsynced nonce ({}!={}+1)".format(
    ...
    def _deal_with_BitcoinCashUSDT(self, ob):
        try:
            bid0 = self.orderbooks["Bitcoin Cash/USDT"]["bids"][0][0]
        except IndexError:
            return ob
        try:
            min_price = (
                min(
                    self.orderbooks["BCH/USDT"]["asks"][0][0],
                    self.orderbooks["BSV/USDT"]["asks"][0][0],
                )
                * 1.03
            )
    ...
    def on_order(self, rr):
        ...
        remaining = (
            amount - filled
            if status not in ("canceled", "filled", "suspended", "expired")
            else 0
        )
        if o is None:
            # (status == "new")
    ...
    def encode(self, request, sub=None):
        ...
        req = {"method": method, "params": p_out, "id": message_id}
