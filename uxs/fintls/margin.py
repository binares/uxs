...
class Position:
    ...
    def __init__(
        ...
        self.liquidation_price = (
            ...
            else self.calc_liquidation_price()
        )
        self.bankrupt_price = (
            bankrupt_price if bankrupt_price is not None else self.calc_bankrupt_price()
        )
    ...
    def calc_liquidation_price(self):
        self.liquidation_price = (
            None if self.price is None else self._calc_liquidation_price()
        )
    ...
    def calc_bankrupt_price(self):
        self.bankrupt_price = (
            None if self.price is None else self._calc_bankrupt_price()
        )
    ...
    def calc_new(
        ...
        if current_avg_price is None:
            ...
            new_avg_price, new_avg_lev = (
                current_avg_price,
                current_avg_leverage if abs(current_amount) > abs(amount) else price,
                leverage,
            )
        else:
            ...
            # Is this right?
            new_avg_lev = new_amount / (
                amount / leverage + current_amount / current_avg_leverage
            )
    ...
    def __add__(self, other):
        if self.symbol != other.symbol:
            raise ValueError(
                "Different symbols: {}, {}".format(self.symbol, other.symbol)
            )
        new_amount, new_avg_price, new_avg_lev = self.calc_new(
            self.amount,
            self.price,
            self.leverage,
            other.price,
