...
FEE_FROM_TARGET = ["binance", "luno", "poloniex"]
COST_LIMIT_WITH_FEE = []
...
class ccxtWrapper:
    ...
    def __init__(
        ...
        *,
        ...
        self._auth_info = dict(
            {x: getattr(self, x, "") for x in self._token_kwds},
    ...
    def _set_market_types(self, markets=None):
        ...
        for market in markets:
            ...
            market.update({"type": type, **{t: (type == t) for t in types}})
            ...
            if market.get("quanto") is None:
                market["quanto"] = (
                    settle not in (market["base"], market["quote"]) if settle else None
                )
    ...
    def set_markets(self, markets, currencies=None):
        ...
        self._set_lot_sizes()
        self._set_market_types()
        default = poll.load_profile(
            "__default__", get_name(self), "markets", verbose=self.verbose
        )
        custom = []
        if getattr(self, "_profile_name", None) is not None:
            custom = poll.load_profile(
                self._profile_name, get_name(self), "markets", verbose=self.verbose
            )
    ...
    def load_markets(self, reload=False):
        ...
        if (not prev_markets or reload) and self.has["fetchTradingFees"]:
            try:
                ...
                logger.error(
                    "{} - could not load trading fees: {}".format(self.id, repr(e))
                )
                logger.exception(e)
        return self.markets
    ...
    def load_trading_fees(self, reload=False):
        if not self.markets:
            return
        no_fees = any(
            m.get("taker") is None or m.get("maker") is None
    ...
    def fetch_markets(self, params={}):
        ...
        for i in range(len(markets)):
            markets[i] = self.deep_extend(
                markets[i],
                self.safe_value(
                    self.options["extendMarkets"], markets[i]["symbol"], {}
                ),
            )
        return markets
    ...
    @staticmethod
    def ob_entry(
        symbol=None, bids=None, asks=None, timestamp=None, datetime=None, nonce=None
    ):
    ...
    def order_entry(
        ...
        **kw
        ...
        if e["cum"] is None:
            ...
            e["cum"] = {"f": 0.0, "r": e["remaining"], "p": 0.0}
    ...
    def convert_order_input(
        ...
        **kw
        ...
        def _round_size(
            symbol, direction, amount, price, takerOrMaker, quotation, **kw
        ):
            return self._prepare_order_size(
                symbol, direction, amount, price, quotation, True, **kw
            )
        ...
        size = _map[as_type](
            symbol, direction, amount, p_valid, takerOrMaker, quotation, **kw
        )

        cost = _cost = payout = None

        if balance is not None:
            _cost = self.calc_cost_from_order_size(
                symbol, direction, size, p_valid, takerOrMaker, method=None
            )
            if side == "buy":
        ...
        if full:
            cost = (
                self.calc_cost_from_order_size(
                    symbol, direction, size, p_valid, takerOrMaker, method=None
                )
                if _cost is None
                else _cost
            )
            payout = self.calc_payout_from_order_size(
                symbol, direction, size, p_valid, takerOrMaker, method=None
            )
    ...
    def get_amount_limits(self, symbol, price):
        ...
        if xcy is None or len(cys) > 1 and cys[1] == xcy:
        ...
        elif (min_xv is not None or max_xv is not None) and getattr(
            self, "tickers", None
        ):
            cys = symbol.split("/")
            ...
            for ln in range(2, 4):
                try:
                    price_in_x = calc_price(
                        (cys[0], xcy), prices, self.load_cy_graph(), ln
                    )
                except RuntimeError:
                    continue
        ...
        for add_to, items, method in [
            (min_from_xv, min_qv_items, "up"),
            (max_from_xv, max_qv_items, "truncate"),
        ]:
            for qv_limit, price_in_x, readjust in items:
                if qv_limit is None:
                    continue
                limit = self.calc_order_size_from_cost(
                    symbol,
                    "buy",
                    qv_limit,
                    price_in_x,
                    quotation="quote",
                    _for_round_amount=True,
                    method=method,
                )
                # print(symbol, qv_limit, price_in_x, qv_limit/price_in_x, limit, limit*price_in_x)
                if readjust:
                    cur_price = self.tickers[symbol]["last"]
                    limit = self.round_amount(
                        symbol, limit * cur_price / price, method=method
                    )
                add_to.append(limit)
        ...
        try:
            _max = min([x for x in [max_v] + max_from_xv if x is not None])
        except ValueError:
            _max = None
    ...
    def round_amount(
        self, symbol, amount, price=None, limit=False, *, method="truncate", **kw
    ):
        inf = self.markets[symbol]
        ...
        if limit:
            if price is None:
                raise ValueError(
                    "`price` must not be `None` if `limit` is set to `True`"
                )
    ...
    def round_price(
        ...
        *,
        ...
        p_round = self.round_entity(
            price, precision, method, precisionMode=self.precisionMode
        )
        if limit_divergence:
            if direction is None:
                raise ValueError(
                    "`direction` must not be `None` if `limit_divergence` is set to `True`"
                )
            if current_price is None:
                raise ValueError(
                    "`current_price` must not be `None` if `limit_divergence` is set to `True`"
                )
            p_round = self.limit_price_by_divergence(
                symbol, direction, price, current_price
            )
        if limit:
            if direction is None:
                raise ValueError(
                    "`direction` must not be `None` if `limit` is set to `True`"
                )
    ...
    def limit_price_by_divergence(self, symbol, direction, price, current_price=None):
        ...
        if (
            not isinstance(maxDivergence, dict)
            or maxDivergence.get(side) is None
        ...
        if direction:
            div_price = self.round_entity(
                current_price * (1 + divergence),
                precision,
                "down",
    ...
    def calc_cost_from_order_size(
        ...
        **kw
        ...
        amount = self._prepare_order_size(
            symbol, direction, amount, price, quotation, True, method, **kw
        )
    ...
    def calc_order_size_from_cost(
        ...
        **kw
        ...
        if direction:
            ...
            if not self.FEE_FROM_TARGET and (
                not _for_round_amount or self.COST_LIMIT_WITH_FEE
            ):
    ...
    def calc_payout_from_order_size(
        self,
        symbol,
        side,
        amount,
        ...
        **kw
        ...
        fee_rate = (
            self.markets[symbol][takerOrMaker]
            if isinstance(takerOrMaker, str)
            else takerOrMaker
        )
        if direction:
    ...
    def calc_order_size_from_payout(
        self,
        symbol,
        side,
        amount,
    ...
    def calc_payout_from_cost(
        ...
        **kw
        ...
        size = self.calc_order_size_from_cost(
            symbol, side, amount, price, takerOrMaker, quotation, method=method, **kw
        )
        payout = self.calc_payout_from_order_size(
            symbol, side, size, price, takerOrMaker, "base", method=None
        )

        return payout

    def calc_cost_from_payout(
        self,
        symbol,
        side,
        amount,
        ...
        **kw
        ...
        size = self.calc_order_size_from_payout(
            symbol, side, amount, price, takerOrMaker, quotation, method=method, **kw
        )
        cost = self.calc_cost_from_order_size(
            symbol, side, size, price, takerOrMaker, "base", method=None
        )
        ...
        **kw
        ...
        if method is not None:
            amount = self.round_amount(
                symbol, amount, price, limit, method=method, **kw
            )

        return amount

    def split_quantity(
        self, symbol, amount, n, evenly=True, limits=(-0.3, 0.3), closed="both"
    ):
        ...
        if not evenly:
            ...
            if lowest_limit < 0:
                limits = [
                    max(x, lowest_limit) if x is not None else None for x in limits
                ]
                split = [
                    self.round_amount(symbol, x)
                    for x in fons.math.series.randomize_summation(
                        amount, n, limits, closed
                    )
                ]
            else:
                evenly = True
    ...
    def convert_volumes(
        ...
        for s, v in tickers.items():
            ...
            if has:
                prices[s] = sum(v[x] for x in has) / len(has)
            elif has_fb:
                prices[s] = sum(v[x] for x in has_fb) / len(has_fb)
        ...
        for s in prices:
            try:
                prices_in_quote[s] = calc_price(
                    "{}/{}".format(s.split("/")[0], quote), prices, graph
                )
            except RuntimeError as e:
                logger2.error(e)

        volumes_in_quote = {
            s: (p * tickers[s]["baseVolume"])
            for s, p in prices_in_quote.items()
        ...
        if sort:
            volumes_in_quote = dict(
                sorted(volumes_in_quote.items(), key=lambda x: x[1], reverse=True)
            )
    ...
    async def create_limited_market_order(
        self,
        symbol,
        side,
        amount,
        ...
        *,
        ...
        if max_spread < 0:
            ...
            raise ValueError(
                "`max_spread` exceeded its hard limit ({}); got: {}".format(
                    MAX_SPREAD_HARD_LIMIT, max_spread
                )
            )
        ...
        oInp = self.convert_order_input(
            symbol, side, amount, price, "taker", quotation, balance=balance
        )

        if not oInp["amount"] or not oInp["price"]:
            raise ccxt.ExchangeError(
                "Invalid amount/price: {}/{}".format(oInp["amount"], oInp["price"])
            )

        return await self.create_limit_order(
            symbol, side, oInp["amount"], oInp["price"]
        )

    async def create_limited_market_buy_order(
        self,
        symbol,
        amount,
        max_spread=0.1,
        ...
        *,
        ...
        return await self.create_limited_market_order(
            ...
            tickers=tickers,
            balance=balance,
            ignore_hard_limit=ignore_hard_limit,
        )

    async def create_limited_market_sell_order(
        self,
        symbol,
        amount,
        max_spread=0.1,
    ...
    async def sell_dust(
        self, quote_order=None, dust_definitions=None, spread_limit=None
    ):
        ...
        if self.verbose:
            logger.debug(
                "{} - positive balances before selling dust: {}".format(
        ...
        for cy, free in balances["free"].items():
            ...
            try:
                market_quote = next(
                    q for q in quote_order if "/".join([cy, q]) in self.markets
                )
            ...
            for quote in q_order2:
                ...
                if dust_definition is None:
                    continue
                elif quote not in self.currencies:
                    continue
            ...
            try:
                await self.create_limited_market_sell_order(
                    symbol, free, spread_limit, tickers=tickers
                )
...
class asyncCCXTWrapper(ccxtWrapper):
    ...
    async def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        # (<class '__main__.kucoin'>, <class '__main__.asyncCCXTWrapper'>,
        #  <class '__main__.ccxtWrapper'>, <class 'ccxt.async_support.kucoin.kucoin'>, ...)
        return await super(ccxtWrapper, self).create_order(
        ...
        if self.own_session and self.session is None:
            ...
            if not self.socks_proxy:
                ...
                connector = aiohttp_socks.ProxyConnector.from_url(
                    self.socks_proxy, ssl=context, loop=self.asyncio_loop
                )
    ...
    if isinstance(xc, ccxt.Exchange):
        try:
            ...
            num_loc = max(
                1,
                next(
                    (i for i, x in enumerate(spl) if any(y.isdigit() for y in x)),
                    len(spl),
                ),
            )
...
def init_exchange(exchange):
    ...
    if isinstance(exchange, dict):
        ...
        D = dict(
            {"exchange": e},
            **{x: y for x, y in exchange.items() if x not in ("exchange", "xc", "e")}
        )
    ...
    if len(D) > 1 or any(kwargs.get(x) for x in token_keywords + ["auth"]) or not e_obj:
        ...
        if isinstance(auth_dict1, str):
            auth_dict1 = {"id": auth_dict1}
        if isinstance(auth_dict2, str):
            auth_dict2 = {"id": auth_dict2}
    ...
    if e_obj is None:
        pass
    elif asyn and not isinstance(e_obj, ccxt.async_support.Exchange):
        e_obj = None
    elif not asyn and isinstance(e_obj, ccxt.async_support.Exchange):
        e_obj = None
    ...
    if not e_obj or e_obj.apiKey != auth.get("apiKey"):
        ...
        if eobj_kw.get("verbose"):
            logger.debug(
                "Initiating ccxt-exchange '{}' with auth_id '{}'".format(
                    e, auth.get("id")
                )
            )
...
def list_exchanges():
    ...
    for attr, v in vars(ccxt).items():
        if (
            isinstance(v, type)
            and issubclass(v, ccxt.Exchange)
