...
class bitget(Exchange):
    ...
    async def fetch_markets(self, params={}):
        ...
        if not len(types):
            types = [self.options['defaultType']]
        result = []
    ...
    async def fetch_markets_by_type(self, type, params={}):
        ...
        elif (type == 'spot') or (type == 'swap'):
            method = type + 'GetInstruments'
            response = await getattr(self, method)(params) or {}
    ...
    async def fetch_tickers_by_type(self, type, symbols=None, params={}):
        ...
        if response['status'] == 'ok':
            response = response['data']['info'] if response['data']['info'] else []
        result = {}
        for i in range(0, len(response)):
    ...
    async def fetch_trades(self, symbol, limit=None, since=None, params={}):
        ...
        result = self.safe_value(result, 'tradeApiDtos', [])
    ...
    async def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        ...
        if since is not None:
            ...
            if limit is not None:
                ...
                raise ArgumentsRequired(self.id + ' fetchOHLCV requires either `limit` or `since` to be defined')
        response = await getattr(self, method)(self.extend(request, params))
        if response['status'] != 'ok':
            return response
    ...
    async def create_order(self, symbol, type, amount, price=None, match_price=0, order_type=0, params={}):
        await self.load_markets()
        market = self.market(symbol)
        request = {
            'instrument_id': market['id'],
    ...
    async def cancel_order(self, id, symbol=None, params={}):
        ...
        result = response if ('data' in response) else self.safe_value(response, market['id'], {})
        #
        #
        # futures, swap
        #
    ...
    async def fetch_orders(self, symbol=None, status=5, since=None, limit=None, params={}):
        if symbol is None:
        ...
        #
        if response['status'] == 'ok':
            response = response['data'].list or []
        else:
            return response
    ...
    async def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        #  '6': incomplete（open+partially filled),
        #  '7': complete（cancelled+fully filled),
        return await self.fetch_orders_by_state('0', symbol, since, limit, params)
    ...
    def sign_sha(self, signParam):
        secretkeySHA = encodeURIComponent(self.hash(self.secret, 'sha1'))
        keyInput = ArrayBuffer(64)
        keyOutput = ArrayBuffer(64)
        for i in range(0, 64):
            if i < len(secretkeySHA):
                keyInput[i] = 0
                keyOutput[i] = 0
            else:
                keyInput[i] = 54
                keyOutput[i] = 92
        for j in range(0, len(secretkeySHA)):
            keyInput[j] = (secretkeySHA[j].charCodeAt()) ^ 0x36
            keyOutput[j] = (secretkeySHA[j].charCodeAt()) ^ 0x5c
        key1 = ArrayBuffer(len(secretkeySHA))
        data1 = ArrayBuffer(len(signParam))
        for i in range(0, len(secretkeySHA)):
            key1[i] = secretkeySHA[i].charCodeAt()
        for i in range(0, len(signParam)):
            data1[i] = signParam[i].charCodeAt()
        keyOutputCopy = ''
        keyInputCopy = ''
        data1Copy = ''
        for xx in range(0, keyOutput.byteLength):
            keyOutputCopy += String.fromCharCode(keyOutput[xx])
        for xx in range(0, keyInput.byteLength):
            keyInputCopy += String.fromCharCode(keyInput[xx])
        for xx in range(0, data1.byteLength):
            data1Copy += String.fromCharCode(data1[xx])
        md5 = self.algo.MD5.create()
        md5.update(keyInputCopy)
        md5.update(data1Copy)
        md = md5.finalize()
        md5 = self.algo.MD5.create()
        md5.update(keyOutputCopy)
        md5.update(md, 0, 16)
        md = md5.finalize().toString(self.enc.Hex)
        return md
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if type == 'public':
            ...
            queryparam = self.sign_sort(query)
            sign = self.sign_sha(queryparam)
            headers = {}
            if method == 'GET':
                url += '?' + queryparam + '&accesskey=' + self.apiKey + '&sign=' + sign + '&req_time=' + timestamp
            else:
                # if isArray or query:
                #     body = self.json(query)
                # }
                body = queryparam
                url += '?accesskey=' + self.apiKey + '&sign=' + sign + '&req_time=' + timestamp
