...
class equos(Exchange):
    ...
    async def fetch_markets(self, params={}):
        ...
        for i in range(0, len(results)):
            markets.append(self.parse_market(results[i]))
        return markets
    ...
    async def fetch_ticker(self, symbol, params={}):
        ...
        if chart is not None:
            return self.parse_ticker(chart, market)
        else:
    ...
    async def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        ...
        if self.timeframes[timeframe] is None:
            raise BadRequest(self.id + ': timeframe ' + timeframe + ' is not supported')
        request = self.extend({
            'pairId': market['id'],
    ...
    async def cancel_order(self, id, symbol=None, params={}):
        ...
        request = self.safe_value(order, 'info')
        request['origOrderId'] = self.safe_value(request, 'orderId')
    ...
    async def edit_order(self, id, symbol, type, side, amount=None, price=None, params={}):
        ...
        if type is None or side is None or amount is None:
            raise ArgumentsRequired(self.id + ': Order does not have enough arguments')
        orgOrder = self.safe_value(order, 'info')
        newOrderRequest = self.create_edit_order_request(orgOrder, market, type, side, amount, price, params)
        response = await self.privatePostCancelReplaceOrder(self.extend(newOrderRequest))
    ...
    def parse_order(self, order, market=None):
        ...
        price = self.convert_from_scale(self.safe_integer(order, 'lastPx', 0), self.safe_integer(order, 'lastPx_scale', 0))
        ...
        average = self.convert_from_scale(self.safe_integer(order, 'price', 0), self.safe_integer(order, 'price_scale', 0))
        cost = None
        if filled != 0:
            if average > 0:
                cost = average * filled
        ...
        if self.safe_integer(order, 'feeTotal') is not None and self.safe_integer(order, 'fee_scale') is not None:
            feeTotal = self.convert_from_scale(self.safe_integer(order, 'feeTotal'), self.safe_integer(order, 'fee_scale'))
    ...
    def parse_trade(self, trade, market):
        ...
        if self.safe_value(trade, 'id') is not None:
            id = self.safe_string(trade, 'id')
            timestamp = self.safe_integer(trade, 'time')
            markeySymbol = self.market(self.safe_string(trade, 'symbol'))
            dateTime = self.iso8601(timestamp)
            ...
            feeCurrency = self.safe_string(trade, 'commissionAsset')
            return {
                ...
                'order': orderId,  # string order id or None/None/null
                'type': type,                    # order type, 'market', 'limit' or None/None/null
