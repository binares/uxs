...
class delta(Exchange):
    ...
    def fetch_markets(self, params={}):
        response = self.publicGetProducts()
        result = []
    ...
    def fetch_order_book(self, symbol, params={}):
        ...
        response = self.publicGetOrderbookIdL2(self.extend({'id': id}, params))
        orderbook = self.parse_order_book(response, None, 'buy_book', 'sell_book', 'price', 'size')
        return orderbook

    def fetch_ticker(self, symbol, params={}):
        ticker = self.publicGetProductsTicker24hr(self.extend(
            {
                'symbol': symbol,
            },
            params
    ...
    def fetch_trades(self, symbol, since=None, limit=50, params={}):
        ...
        response = self.publicGetOrderbookIdL2(self.extend({'id': market['id']}, params))
    ...
    def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        ...
        #}
        response = self.publicGetChartHistory(request)
        if response['s'] == 'ok':
    ...
    def fetch_balance(self, params={}):
        wallets = self.privateGetWalletBalances()
        response = {}
    ...
    def get_orders(self, symbol, params={}):
        self.load_markets()
        request = {}
        market = self.market(symbol)
    ...
    def cancel_order(self, id, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.privateDeleteOrders(self.extend(
            {
                'id': id,
                'product_id': market['id'],
            },
    ...
    def fetch_order_leverage(self, symbol):
        self.load_markets()
        response = self.privateGetOrdersLeverage({
            'product_id': self.market_id(symbol),
        })
        return response

    def change_position_margin(self, symbol, deltaMargin):
        self.load_markets()
        response = self.privatePostPositionsChangeMargin({
            'product_id': self.market_id(symbol),
            'delta_margin': deltaMargin,
        })
        return response

    def change_order_leverage(self, symbol, leverage):
        self.load_markets()
        response = self.privatePostOrdersLeverage({
            'product_id': self.market_id(symbol),
            'leverage': leverage,
        })
        return response
