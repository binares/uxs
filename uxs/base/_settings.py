    ...
    try:
        ...
        with open(SETTINGS_PATH, encoding="utf-8") as f:
            ...
            if settings is None:
                settings = {}
            settings = {
                k: (v if v is not None else deepcopy(_SETTINGS_INITIAL[k]))
...
APPLY = {
    "ENABLE_CACHING": _prepare_enable_caching,
...
for _ in SETTINGS:
    exec(
        """def get_{name_l}():
        return get_setting('{name}')""".format(
            name_l=_.lower(), name=_
        )
    )

    exec(
        """def set_{name_l}(value, make_permanent=False):
        return set({{'{name}': value}}, make_permanent)['{name}']""".format(
            name_l=_.lower(), name=_
        )
    )
...
def clear_setting(param, make_permanent=False):
    ...
    for i, x in enumerate(deep_param):
        _d[x] = (
            {}
            if i < ln - 1
            else deepcopy(deep_get([_SETTINGS_INITIAL], deep_param, return2=DEL))
        )
        _d = _d[x]
