...
class bitmexPosition(Position):
    def _calc_bankrupt_price(self, *, round=True):
        ...
        bankrupt_price = self.price / (
            1 + mp * self._initial_margin(self.leverage, taker_fee)
        )
        if round:
            method = "up" if self.amount > 0 else "down"
            bankrupt_price = self.api.round_price(
                self.symbol, bankrupt_price, method=method
            )
    ...
    def _calc_liquidation_price(self):
        ...
        liq = bankrupt_price + mp * (
            self.price * (maintenance_margin + mp * funding_rate)
        )
    ...
    def verify_is_perpetual(self):
        if self.symbol not in ("BTC/USD", "ETH/USD"):
            raise ValueError(
                "The contract must be perpetual swap. Got symbol: {}".format(
                    self.symbol
                )
            )
...
if __name__ == "__main__":
    ...
    data = {
        ...
        3: [
            ...
            7613,
            2,
        ],  # 4839.5 | 4847.0 (the current amount was on 2x lev)
