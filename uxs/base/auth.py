...
def _verify_exchange_entry(entry, exchange, i):
    ...
    if len(missing):
        ...
        if i not in _we:
            logger.warning(
                "Token {} of exchange '{}' missing the following params: {}".format(
...
def _update_entry_by_its_id(credentials):
    ...
    not_already_defined = {
        x: y for x, y in interpretation.items() if credentials.get(x) is None
    }
...
def read_tokens(password=None):
    ...
    if tokens_path is not None:
        ...
        tokens = {
            xc: [e for i, e in enumerate(entries) if _verify_exchange_entry(e, xc, i)]
...
def get_auth2(
    ...
    **kw
    ...
    if xc_lower not in tokens:
        raise ValueError(
            "Exchange '{}' has no tokens associated with it.".format(exchange)
        )
    ...
    _specs = {
        k: requested[k] for k in ["test", "info", "trade", "withdraw", "user", "active"]
    }
    ...
    if not len(matches):
        raise ValueError(
            "No {} token matched against these params: {}".format(exchange, show_specs)
        )
    elif any(x not in matches[0] for x in required):
        match_id = matches[0].get("id")
        raise TypeError(
            "Token {}:{} is missing one of these properties: {}".format(
                xc_lower, match_id, required
            )
        )
    ...
    tlogger0.debug(
        "Retrieved auth id '{}' (called from '/{}:{}')".format(
            entry.get("id"),
            "/".join(f.f_code.co_filename.split(os.sep)[-3:]),
            f.f_code.co_name,
        )
    )
...
def encrypt_tokens(password, destination_dir=None, **kw):
    ...
    _pth = (
        os.path.join(destination_dir, os.path.basename(tokens_path))
