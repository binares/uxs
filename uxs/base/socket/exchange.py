...
class ExchangeSocket(WSClient):
    ...
    def __init__(self, config={}):
        ...
        is_test = (
            "test" in config and config["test"] or "test" not in config and self.test
        )
        if is_test:
        ...
        self.api = init_exchange(
            dict(_params, kwargs=dict(_kwargs, load_cached_markets=load_cached_markets))
        )
        ...
        try:
            ...
            if self.use_pro:
                raise ccxt.NotSupported(
                    self.exchange + " is not supported by ccxt pro yet."
                )
    ...
    def _init_has(self):
        ...
        for x in [
            ...
            "edit_order",
            "cancel_order",
        ] + list(emulation_methods):
            x2 = x if not x.startswith("own") else x.split("_")[-1]
            which_acc = (
                None
                if x2 not in under_account
                else ("account" if not x.startswith("own") else "own_market")
            )
            ...
            for k in ("ws", "emulated", "ccxt"):
                try:
                    initial_vals[k] = (
                        self.has[x]
                        if x not in under_account
                        else self.has["account"][x]
                    ).pop(k)
                except Exception:
                    pass
            ...
            if ws_has is None:
                ws_has = (
                    self_has if x in emulation_methods and emulated is None else False
                )
            if ccxt_has is None:
                ccxt_has = (
                    None
                    if x in emulation_methods
                    else any(self.api.has.get(cm) for cm in camel_methods)
                )
            if emulated is None and not ws_has:
                if x == "orderbook" or (x == "trades" and self.has_got("l3", "trades")):
                    ...
                    if (
                        x == "own_market_order"
                        and self.has_got("account", "order", "ws")
                        or x == "own_market_fill"
                        and self.has_got("account", "fill", "ws")
                    ):
                        emulated = "account"
                    elif (
                        self.has_got("trades", "ws")
                        and self.has_got("trades", "orders")
                        or self.has_got("l3", "ws")
                        and self.has_got("l3", "trades", "orders")
                    ):
                        emulated = "trades"
                if emulated is None and x in emulation_methods:
                    emulated = "poll" if any(self.has_got(m) for m in methods) else None
                    if x == "balance" and not self.has_got(
                        "fetch_balance", ["free", "used"]
                    ):
                        emulated = None
            ...
            update = {
                x2: {"_": any_has, "ws": ws_has, "emulated": emulated, "ccxt": ccxt_has}
            }
        ...
        if self.has_got("account", "balance", "ws"):
            self.has["account"]["balance"].update(
                {
                    k: True
                    for k in self.BALANCE_KEYWORDS
                    if k not in self.has["account"]["balance"]
                }
            )

        for x in ("fill", "order"):
            if (
                self.has_got("account", x, "emulated")
                and not self.has_got("account", x, "ws")
                and self.has_got("own_market", x, "ws")
            ):
                self.has["account"][x].update(
                    {"_": False, "ws": False, "emulated": None}
                )

        self.has.update(
            {
                x: _copy.deepcopy(deep_get([self.has], ["account", x]))
        ...
        deep_update(
            self.has,
            {
                x: _copy.deepcopy(deep_get([self.has], ["own_market", x]))
                for x in ["order", "fill"]
            },
        )
        ...
        for acc in ("account", "own_market"):
            emulated = unique(
                (
                    self.has[acc][x]["emulated"]
                    for x in under_account
                    if self.has_got(acc, x, "emulated")
                ),
                astype=list,
            )
            if not any(emulated):
            ...
            any_independent = any(
                self.has_got(acc, x, "ws") and not self.has_got(acc, x, "emulated")
                for x in under_account
            )
            if (
                emulated
                and not any_independent
                and self.channels[acc].get("url") is None
            ):
                self.channels[acc]["url"] = ""

        # OHLCV and timeframes
        deep_update(self.has, {"ohlcv": {"_": self.has_got("ohlcv"), "timeframes": {}}})
        ...
        if self.has_got("ohlcv"):
            ...
            self.timeframes = {
                x: y for x, y in self.timeframes.items() if y != "__del__"
            }
        ...
        for x, kw in [
            ...
            if self.has_got(x, "emulated"):
                ...
                source_dict = (
                    self.has[emulation_methods[x][0]]
                    if self.has[x]["emulated"] != "l3"
                    else self.has["l3"][x]
                )
                update = {
                    k: source_dict.get(k, None)
                    for k in keywords
                    if k in source_dict or k not in self.has[x]
                }
    ...
    def setup_test_env(self, *args):
        ...
        if not self.url_components.get("test"):
            raise NotImplementedError(
                "{} - test env is not implemented yet".format(self.__class__.__name__)
            )
    ...
    def notify_unknown(self, r, max_chars=500, logger=logger2):
        logger.warning(
            "{} - unknown response: {}".format(self.name, str(r)[:max_chars])
        )
        ...
        for dependency in ("l3", "trades"):
            if _is_in(dependency, emulated) and self.has_got(dependency):
                if "symbol" not in self.get_value(s.channel, "required"):
                    raise ValueError(
                        "{} - {} can't assign dependency '{}' due to it not having 'symbol' param".format(
                            self.name, s, dependency
                        )
                    )
    ...
    def update_tickers(
        self, data, *, action="update", set_event=True, enable_sub=False
    ):
        """
        :param data:
            [
        ...
        for d in data:
            ...
            cb_input = {"_": "ticker", "symbol": symbol, "data": changes}
    ...
    def create_orderbooks(self, data, *, set_event=True, enable_sub=False):
        ...
        for ob in data:
            symbol = ob["symbol"]
            self.orderbooks[symbol] = new = self.orderbook_maintainer._deep_overwrite(
                create_orderbook(ob)
            )
    ...
    def update_orderbooks(
        self, data, *, set_event=True, enable_sub=False, is_delta=False
    ):
        """
        :param data:
            [
        ...
        for d in data:
            ...
            for side, uniq_changes in zip(
                ("bids", "asks"), (uniq_bid_changes, uniq_ask_changes)
            ):
                branch = self.orderbooks[symbol][side]
                for item in d[side]:
                    p, a0, a = update_branch(
                        item, branch, side, is_delta=is_delta, round_to=amount_pcn
                    )
                    try:
                        a0 = uniq_changes.pop(p)[1]
                    except KeyError:
                        pass
    ...
    def _update_tickers_from_ob(self, symbols):
        ...
        does_send = sends and (
            not isinstance(sends, dict) or "_" not in sends or sends["_"]
        )
        all_subbed = self.is_subscribed_to({"_": "all_tickers"}, 1)
        only_if_not_subbed = isinstance(sends, dict) and sends.get(
            "only_if_not_subbed_to_ticker"
        )
        if not does_send or only_if_not_subbed and all_subbed:
            return
        set_ticker_event = not isinstance(sends, dict) or sends.get(
            "set_ticker_event", True
        )
        is_subbed = lambda symbol: self.is_subscribed_to(
            {"_": "ticker", "symbol": symbol}, 1
        )
    ...
    def create_l3_orderbooks(self, data, *, set_event=True, enable_sub=False):
        ...
        for l3 in data:
            symbol = l3["symbol"]
            amount_pcn = self.markets.get(symbol, {}).get("precision", {}).get("amount")
            self.l3_books[symbol] = new = self.l3_maintainer._deep_overwrite(
                create_l3_orderbook(l3)
            )
        ...
        for d in data:
            ...
            if set_event:
                self.safe_set_event(
                    "ohlcv",
                    (symbol, timeframe),
                    {"_": "ohlcv", "symbol": symbol, "timeframe": timeframe},
                )
    ...
    def update_positions(self, data, *, set_event=True, enable_sub=False):
        ...
        for d in data:
            ...
            if set_event:
                self.safe_set_event(
                    "position", symbol, {"_": "position", "symbol": symbol}
                )

            cb_input = {"_": "position", "symbol": symbol, "data": changes}
    ...
    def update_balances(self, data, *, set_event=True, enable_sub=False):
        ...
        def parse_dict(x):
            ...
            if "total" in x:
                ...
                if (
                    "free" not in new
                    and new.get("used") is not None
                    and new["total"] is not None
                ):
                    new["free"] = new["total"] - new["used"]
                if (
                    "used" not in new
                    and new.get("free") is not None
                    and new["total"] is not None
                ):
        ...
        for x in deltas:
            ...
            if isinstance(x, dict):
                ...
                try:
                    ...
                    pass
            try:
                d = self.balances[symbol]
            except KeyError:
                d = {"free": 0, "used": 0, "total": 0}
    ...
    def update_balances_from_order_delta(self, o_new, o_prev={}):
        diffs = {
            k: (o_new[k] - o_prev[k] if o_prev.get(k) is not None else o_new[k])
        ...
        data = [(cy_in, d_in_free, d_in_used), (cy_out, d_payout, 0)]
    ...
    def add_order(
        ...
        *,
        ...
        if self.is_order_balance_updating_enabled(o):
            try:
                self.update_balances_from_order_delta(o)
    ...
    def add_order_from_dict(self, d, *, set_event=True, enable_sub=False, drop=None):
        if d["id"] in self.orders:
            return self.update_order_from_dict(
                d, set_event=set_event, enable_sub=enable_sub, drop=drop
            )
        if drop is not None:
            d = self.drop(d, drop)
    ...
    def update_order_from_dict(self, d, *, set_event=True, enable_sub=False, drop=None):
        ...
        self.update_order(
            **kw, params=params, set_event=set_event, enable_sub=enable_sub
        )
        ...
        *,
        ...
        if was_dict:
            ...
            if o.get("cum") is None:
                o["cum"] = {"f": 0.0, "r": o["remaining"], "p": 0.0}
        ...
        if remaining is not None:
            ...
            try:
                ...
                smallest_allowed = (
                    pow(10, (-1) * precision)
                    if self.api.precisionMode != ccxt.TICK_SIZE
                    else precision
                )
        ...
        def cumulate(d, r=remaining, f=filled, p=payout, c=cost, l=lastTradeTimestamp):
            for name, al, value, op in [
                ("remaining", "r", r, modify_remaining),
                ("filled", "f", f, max),
                ("payout", "p", p, max),
                ("cost", "c", c, max),
        ...
        if self.is_order_balance_updating_enabled(o):
            try:
                self.update_balances_from_order_delta(o, o_prev)
            except Exception as e:
                logger.exception(e)
        ...
        if set_event:
            self.safe_set_event(
                "order", id, {"_": "order", "symbol": o["symbol"], "id": id}
            )
    ...
    def close_order_assume_filled(self, order):
        ...
        if o["price"] is not None and o["remaining"]:
            added_payout = self.api.calc_payout(
                o["symbol"], o["side"], o["remaining"], o["price"]
            )
    ...
    def add_fill(
        ...
        *,
        ...
        if (symbol is None or side is None) and order is None:
            raise ValueError(
                "{} - fill id: {}. Both symbol and side must be "
        ...
        if loc is not None:
            if warn:
                self.log2(
                    "fill {} already registered.".format(
                        (id, symbol, side, price, amount, order)
                    )
                )
            if enable_sub:
        ...
        def insert_fill(f):
            ...
            if set_event:
                self.safe_set_event(
                    "fill",
                    order,
                    {"_": "fill", "symbol": symbol, "order": order, "id": id},
                )
        ...
        if order is None:
            ...
            is_new = (
                not o["lastTradeTimestamp"] or o["lastTradeTimestamp"] < f["timestamp"]
            )
            ...
            if self.order["update_payout_on_fill"]:
                payout = (
                    self.api.calc_payout(
                        symbol, o["side"], amount, price, fee_rate, fee=fee
                    )
                    if payout is None
                    else payout
                )
            ...
            if o_params:
                self.update_order(
                    order, **o_params, from_fills=True, set_event=set_order_event
                )

        elif not any(
            x["id"] == id and x["symbol"] == symbol
            for x in self.unprocessed_fills[order]
        ):
            self.log2(
                "adding {} to unprocessed_fills".format(
                    (id, symbol, type, side, price, amount, order)
                )
            )
    ...
    def add_fill_from_trade(
        self, t, *, set_event=True, set_order_event=True, enable_sub=False, warn=False
    ):
        orders = t.get("orders")
        if not orders:
            return
        for order, takerOrMaker in zip(orders, ["maker", "taker"]):
            ...
            cost, fee = (
                t.get("cost"),
                t.get("fee") if takerOrMaker == t.get("takerOrMaker") else None,
                None,
            )
            fill = dict(t, order=order, takerOrMaker=takerOrMaker, cost=cost, fee=fee)
            self.add_fill_from_dict(
                fill,
                set_event=set_event,
                set_order_event=set_order_event,
                enable_sub=enable_sub,
    ...
    def fetch_data(self, s, prev_state):
        ...
        if s.state and not prev_state and attr is not None and self.has_got(attr):
    ...
    async def fetch_data_loop(self):
        ...
        def get_non_emulated_subs():
            return [
                ...
                if not self.has_got(s.channel, "emulated")
                or not self._is_poll(self.has[s.channel]["emulated"])
            ]
    ...
    async def poll_loop(self):
        ...
        def get_emulated_subs():
            return [
                s
                for s in self.subscriptions
                if self.has_got(s.channel, "emulated")
                and self._is_poll(self.has[s.channel]["emulated"])
            ]
        ...
        def change_states():
            ...
            for s in get_emulated_subs():
                ...
                mmi = deep_get(
                    [self.max_missed_intervals],
                    methods.get(s.channel),
                ...
                if now - self._last_fetches[id] > mmi * interval:
                    ...
                    if s.channel == "l3" and self.l3_maintainer.is_subbed_to_l2_ob(
                        symbol
                    ):
        ...
        while self.is_running():
            ...
            account_sub = (
                self.get_subscription(("account",))
                if self.is_subscribed_to(("account",))
                else None
            )
            own_market_subs = [
                s for s in self.subscriptions if s.channel == "own_market"
            ]

            if account_sub:
                id_bal = ("fetch_balance",)
                if self._is_poll(
                    self.has["account"]["balance"]["emulated"]
                ) and self._is_time("fetch_balance", id_bal):
                    coros["balance"] = self.poll_fetch_and_activate(
                        account_sub, "fetch_balance", id=id_bal
                    )
    ...
    async def poll_orders(self):
        def get_order_fetch_methods():
            ...
            if self.has_got("fetch_my_trades"):
                ...
                if self.has_got("fetch_order"):
                    ...
                    methods = [
                        "fetch_open_orders"
                    ]  # this is going to be a bit tricky one
            elif self.has_got("fetch_order"):
                methods = ["fetch_order"]
            return methods

        has_account_order_emul = self._is_poll(self.has["account"]["order"]["emulated"])
        has_own_market_order_emul = self._is_poll(
            self.has["own_market"]["order"]["emulated"]
        )
        ...
        def filter_orders_by_last_update(orders, method):
            interval = deep_get(
                [self.intervals], method, return2=self.intervals["default"]
            )
            return [
                o
                for o in orders
                if time.time() - o["lastUpdateTimestamp"] / 1000 > interval
            ]
            ...
            if method == "fetch_order":
                ...
                symbols = unique(
                    (
                        o["symbol"]
                        for o in orders_to_be_probed
                        if self._is_time(method, _id(o["symbol"]))
                    ),
                    astype=list,
                )
                for symbol in symbols:
                    ...
                    if subs:
                        coros[(method, None, symbol)] = self.poll_fetch_and_activate(
                            subs, method, (symbol,), _id(symbol)
                        )
            elif all_subs and self._is_time(method, _id(None)) and orders_to_be_probed:
                coros[(method, None, None)] = self.poll_fetch_and_activate(
                    all_subs, method, id=_id(None)
                )
            return coros
        ...
        async def exec_coros(coros):
            if not coros:
                return [], []
            nametuples_by_futs = {
                asyncio.ensure_future(coro): name for name, coro in coros.items()
            }
        ...
        for i, method in enumerate(methods):
            ...
            orders_to_be_probed = filter_orders_by_last_update(
                orders_to_be_probed, method
            )
            ...
            orders_to_be_probed = [
                o
                for o in orders_to_be_probed
                if any(nt[2] is None or nt[2] == o["symbol"] for nt in successful)
    ...
    async def poll_fetch_and_activate(self, subscription, method, args=None, id=None):
        ...
        try:
            ...
            self.log(
                "error occurred while polling {}{} - {}".format(method, args, repr(e))
            )
            self.log(e)
            return e
        finally:
        ...
        for s in subs:
            ...
            s = next(
                (_s for _s in self.subscriptions if _s.id_tuple == s.id_tuple), None
            )
            if s is None:
                continue
            if s.channel in ("orderbook", "l3"):
    ...
    def get_current_price(
        self, symbol, components=["last", "bid", "ask"], *, active=True, as_dict=False
    ):
        has_tAll_bidAsk = self.has_got(
            "all_tickers", ("bid", "ask")
    ...
    def get_markets_with_active_tickers(self, last=True, bidAsk=True):
        ...
        if has_t:
            from_ticker.update(
                s.params["symbol"]
                for s in self.sh.subscriptions
    ...
    async def fetch_balance(self, params={}):
        if not self.test:
            balances = await poll.fetch(
                self.api, "balances", 0, kwargs={"params": params}
            )
        else:
    ...
    async def fetch_l3_order_book(self, symbol, limit=None):
        raise ccxt.NotSupported(
            self.exchange + " does not support fetchL3OrderBook yet"
        )

    async def fetch_tickers(self, symbols=None, params={}):
        if not self.test:
            tickers = await poll.fetch(
                self.api, "tickers", 0, kwargs={"symbols": symbols, "params": params}
            )
        else:
            tickers = await self.api.fetch_tickers(symbols, params)
        self.update_tickers([y for x, y in tickers.items() if x != "info"])
        return tickers

    async def fetch_ticker(self, symbol, params={}):
        if not self.test:
            ticker = await poll.fetch(
                self.api, ("ticker", symbol), 0, kwargs={"params": params}
            )
        else:
    ...
    async def fetch_ohlcv(
        ...
        kwargs = {"since": since, "limit": limit, "params": params}
        if not self.test:
            ohlcv = await poll.fetch(
                self.api, ("ohlcv", symbol, timeframe), 0, kwargs=kwargs
            )
        else:
    ...
    async def fetch_orders(
        self, symbol=None, since=None, limit=None, params={}, *, skip=False
    ):
    ...
    async def query_order(self, id, symbol):
        ...
        if self.has_got("fetch_my_trades"):
            ...
            o = {
                "id": id,
                "symbol": symbol,
                "trades": [t for t in trades if t["order"] == id],
            }
        ...
        elif self.has_got("fetch_orders", "filled") or self.has_got(
            "fetch_open_orders", "filled"
        ):
            method = next(
                x
                for x in ["fetch_orders", "fetch_open_orders"]
                if self.has_got(x, "filled")
            )
            symbolRequired = self.has_got(method, "symbolRequired")
            _args = [symbol] if symbolRequired else []
            self.log("querying order {} {} via {}".format(id, symbol, method))
            orders = await getattr(self, method)(
                *_args, skip=[id]
            )  # update all except this order
    ...
    async def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        if price is not None:
            ...
            price = self.api.round_price(
                symbol,
                price,
                side,
                True,
                limit_divergence=True,
                current_price=(cp["bid"], cp["ask"]),
            )
            if (
                not price and type != "market"
            ):  # raise error as price `0` / `None` might accidentally create a market order
                raise ccxt.InvalidOrder(
                    "{} - {} order price {} is limited to 0".format(
                        self.name, symbol, price
                    )
                )
        if self.verbose and self.verbose >= 0.5:
            self._log(
                "creating order {}".format((symbol, type, side, amount, price, params))
            )
        if self.has_got("create_order", "ws") and self.is_active():
            ...
            try:
                ...
                self.log_error(
                    "could not determine if order {} is immediate fill".format(order), e
                )
            ...
            try:
                if self.is_order_querying_enabled(
                    order, is_immediate_fill=immediate_fill
                ):
                    q = await self.query_order(r["id"], symbol)
                    if q == "closed":
                        _o = self.api.order_entry(
                            **dict(
                                order,
                                **{
                                    k: v
                                    for k, v in r.items()
                                    if k not in order or v is not None
                                }
                            )
                        )
    ...
    async def cancel_order(self, id, symbol=None, params={}):
        ...
        if self.has_got("cancel_order", "ws") and self.is_active():
            ...
            _symbol = (
                ...
                else deep_get([self.orders], [id, "symbol"])
            )
            cancel_automatically = self.is_order_auto_canceling_enabled(_symbol)
            if (
                cancel_automatically
                and not self._is_subbed_to_order_ws(_symbol)
                and self.has_only_fetch_open_orders()
            ):
                _args = (
                    [_symbol]
                    if self.has_got("fetch_open_orders", "symbolRequired")
                    else []
                )
                self.log(
                    "pre querying order {} {} via fetch_open_orders before canceling".format(
                        id, _symbol
                    )
                )
            ...
            try:
                r = await self.api.cancel_order(id, symbol, params)
    ...
    async def edit_order(self, id, symbol, *args):
        ...
        new_side = (
            args[1] if len(args) > 1 and args[1] is not None else o_prev.get("side")
        )
        # amount
        ...
        if len(args) >= 4 and args[3] is not None:
            cp = self.get_current_price(symbol, as_dict=True)
            new_price = self.api.round_price(
                _symbol,
                args[3],
                new_side,
                True,
            ...
            if (
                not new_price and new_type != "market"
            ):  # raise error as price `0` / `None` might accidentally create a market order
                raise ccxt.InvalidOrder(
                    "{} - {} order price {} is limited to 0".format(
                        self.name, _symbol, new_price
                    )
                )
        ...
        if self.has_got("edit_order", "ws") and self.is_active():
            ...
            prev_amount = (
                o_prev["remaining"]
                if r.get("id") != id and o_prev.get("remaining") is not None
                else o_prev.get("amount")
            )
            new_amount = (
                order["amount"] if order.get("amount") is not None else prev_amount
            )
            order.update(
                {
            ...
            try:
                if self.api.has[
                    "editOrder"
                ] != "emulated" and self.is_order_querying_enabled(
                    order, "edit_order", is_immediate_fill=immediate_fill
                ):
                ...
                self.log_error(
                    "could not query order {} {}".format(order["id"], _symbol), e
                )
    ...
    def add_ccxt_order(self, response, order={}, from_method="create_order", skip=None):
        ...
        parsed_filtered = {
            k: v
            for k, v in parsed.get("order", {}).items()
            if v is not None or k not in order
        }
        ...
        if not skip:
            ...
            if (
                from_method == "edit_order"
                and o
                and o["remaining"]
                and final_order.get("remaining")
            ):
    ...
    def parse_ccxt_order(self, r, *args):
        ...
        if o.get("amount") is not None:
            if o.get("filled") is not None and o.get("remaining") is None:
                o["remaining"] = max(0.0, o["amount"] - o["filled"])
            elif o.get("remaining") is not None and o.get("filled") is None:
                o["filled"] = max(0.0, o["amount"] - o["remaining"])

        if o.get("status"):
            if o["status"] not in ("open", "expired"):
                o["remaining"] = 0.0

        params = self.api.lazy_parse(
            o, ("symbol", "side"), {"amount": "filled", "price": ["average", "price"]}
        )

        if None not in params.values() and params["price"] != 0:
            params.update(
                {x: o[x] for x in ("takerOrMaker", "fee") if o.get(x) is not None}
            )
            try:
    ...
    async def safely_mark_order_closed(self, id):
        ...
        if self.has_got("fetch_my_trades") and self.order["update_filled_on_fill"]:
            self.log(
                "final querying order {} {} via fetch_my_trades".format(id, symbol)
            )
            _args = (
                [symbol] if self.has_got("fetch_my_trades", "symbolRequired") else []
            )
            try:
                await self.fetch_my_trades(*_args)
            except Exception as e:
                self.log_error(
                    "error fetching my trades to mark order {} {} as closed".format(
                        id, symbol
                    ),
                    e,
                )
            ...
            try:
                o2 = await self.fetch_order(id, symbol, skip=True)
            except Exception as e:
                self.log_error(
                    "error fetching order {} {} to mark it as closed".format(
                        id, symbol
                    ),
                    e,
                )
            ...
            method = (
                "fetch_orders"
                if self.has_got("fetch_orders")
                else "fetch_closed_orders"
            )
            self.log("final querying order {} {} via {}".format(id, symbol, method))
            _args = [symbol] if self.has_got(method, "symbolRequired") else []
            try:
                orders = await getattr(self, method)(*_args, skip=[id])
            except Exception as e:
                self.log_error(
                    "error invoking {} to mark {} {} as closed".format(
                        method, id, symbol
                    ),
                    e,
                )
    ...
    def is_immediate_fill(self, order):
        ...
        is_st_subbed = self.is_subscribed_to(("ticker", symbol), True) and self.has_got(
            "ticker", ["ws", "bid", "ask"]
        )
        is_at_subbed = self.is_subscribed_to(("all_tickers"), True) and self.has_got(
            "all_tickers", ["ws", "bid", "ask"]
        )
    ...
    def is_order_querying_enabled(
        self, order, method="create_order", *, is_immediate_fill=None
    ):
        """:type order: dict"""
        ...
        if self.has_got(method, "filled"):
            return False
        elif self.has_got("account", "order", "ws") and self.is_subscribed_to(
            ("account",), True
        ):
            return False
        elif self.has_got("own_market", "order", "ws") and self.is_subscribed_to(
            ("own_market", symbol), True
        ):
            return False
        if type == "market":
            return True
    ...
    def is_order_balance_updating_enabled(self, order=None):
        ...
        if (
            order is None
            or not order["lastTradeTimestamp"]
            or not self._last_fetches["fetch_balance"]
            or order["lastTradeTimestamp"] < self._last_fetches["fetch_balance"] * 1000
        ):
            return False
        if isinstance(ub, str):
            return ub == "if-not-subbed-to-balance" and (
                not self.has_got("balance", "ws")
                or not self.is_subscribed_to(("account",), True)
            )
        else:
            return bool(ub)
    ...
    def guess_symbol(self, symbol, direction=1):
        ...
        if not isinstance(symbol, str):
            raise TypeError(
                "Symbol must be of type <str>, got: {}".format(type(symbol))
            )
        sep = self.symbol["sep"]
        ...
        if not direction:
            if not sep:
                ...
                ln = next(
                    (
                        len(q)
                        for q in self.symbol["quote_ids"]
                        if getattr(symbol, method)(q)
                    ),
                    None,
                )
                if ln is None:
    ...
    def convert_timeframe(self, timeframe, direction=1):
        ...
        if not direction:
            tf = next(
                (common for common, id in self.timeframes.items() if id == timeframe),
                None,
            )
        else:
    ...
    def convert_cnx_params(self, params):
        ...
        def _get_value(key, default=None):
            return self.get_value(
                channel, ["cnx_params_converter_config"] + key, default
            )
        ...
        def _change_case(s, method="lower"):
            ...
            return (
                getattr(s, method)()
                if isinstance(s, str)
                else cls(getattr(x, method)() if x is not None else x for x in s)
            )
        ...
        def _extract_base_quote(symbol):
            ...
            if cy_pairs:
                if "base_cy" in include:
                    params["base_cy"] = (
                        cy_pairs[0][0] if is_str else cls(p[0] for p in cy_pairs)
                    )
                if "quote_cy" in include:
                    params["quote_cy"] = (
                        cy_pairs[0][1] if is_str else cls(p[1] for p in cy_pairs)
                    )
    ...
    def _reload_markets(self, e=None):
        ...
        if (
            reload_interval is not None
            and time.time() - self._last_markets_loaded_ts > reload_interval
        ):
    ...
    def fill_in_params(self, strings, symbols=None, timeframes=None, currencies=None):
        ...
        replaced = self.fill_in(
            replaced, "<timeframe>", timeframes, self.convert_timeframe
        )
    ...
    def dict_update(new, prev, info=True, copy=False):
        ...
        if not isinstance(prev, dict) or not isinstance(new, dict):
            ...
            if (
                ...
                and "info" in new
                and isinstance(new["info"], dict)
            ):
                new_dict["info"] = ExchangeSocket.dict_update(
                    new["info"], prev_info, False, copy
                )
            return new_dict
    ...
    def list_new_only(new_seq, prev_seq, timestamp_key, sort=False):
        ...
        try:
            ...
            from_i = next(
                (i for i, x in enumerate(new_seq) if x[timestamp_key] > last_ts),
                len(new_seq),
            )
            if from_i:
