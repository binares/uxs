...
class OrderbookMaintainer:
    ...
    def __init__(self, xs):
        ...
        for i, cfg in enumerate(
            [xs.connection_defaults] + list(xs.connection_profiles.values())
        ):
            ...
            if (
                self.cfg["force_create"] is not None
            ):  # and not cfg['receives_snapshot']:
        ...
        for symbol in symbols:
            if self.data.get(symbol) is None:
                ...
                for hold, id in holds[symbol]:
                    asyncio.ensure_future(self._push_cache_after(symbol, hold, id))
    ...
    async def _fetch_and_create(self, symbol):
        try:
            ...
            self.xs._log(
                "error occurred while fetching and creating {} {} - {}".format(
                    self.name, symbol, repr(e)
                )
            )
            self.xs._log(e)
        else:
    ...
    def _assign(self, ob):
        ...
        if self.cfg["uses_nonce"] and ob["nonce"] is None:
            raise ValueError(
                "{} '{}' can't be assigned because it is missing nonce".format(
                    self.name, symbol
                )
            )
        ...
        if is_synced and not performed_update:
            ...
            method(
                [{"symbol": symbol, "bids": [], "asks": [], "nonce": ob["nonce"]}],
                enable_sub=True,
            )
    ...
    def _play_out(ob, to_push, side):
        ...
        ob_extremum = next(
            (price for price, _ in ob[side] if price not in nullified), None
        )
    ...
    def _push_cache(self, symbol, force_till_id=None):
        ...
        if uses_nonce:
            start_from = next(
                (
                    len(updates) - i
                    for i, u in enumerate(reversed(updates))
                    if self.resolve_nonce(u["nonce"])[1] <= cur_nonce
                ),
                0,
            )
        ...
        if force_till_id not in (None, -1):
            id_loc = next(
                (i for i, u in enumerate(eligible) if u["__id__"] == force_till_id), -1
            )
        ...
        if force_till_id != -1:
            ...
            include = next(
                (
                    i
                    for i, u in enumerate(eligible[id_loc + 1 :])
                    if "__hold_until__" in u and u["__hold_until__"] > now
                ),
                None,
            )
            up_to = include
        ...
        def _reset(method, reason):
            if self._is_time(symbol, method):
                self.xs.log(
                    "{}ing {} {} due to {}.".format(method, self.name, symbol, reason)
                )
        ...
        if cur_nonce != ob["nonce"] or to_push.get("bids") or to_push.get("asks"):
            ...
            getattr(self.xs, self.update_method)(
                [to_push], enable_sub=is_synced
            )  # update the orderbook
        # print(ob['nonce'])

        if not uses_nonce:
            # Not dropping them would result in them being re-counted as "eligible_nonce" afterwards
            self.cache[symbol]["updates"] = (
                eligible_nonce[up_to:] if up_to is not None else []
            )
    ...
    def _renew(self, symbol, method="reload"):
        ...
        if method not in ("reload", "restart"):
            raise ValueError(
                "{} - incorrect ob renew method: {}".format(self.xs.name, method)
            )
        if method == "reload":
            self._schedule_creation(symbol)
        else:
    ...
    def _is_time(self, symbol, method="reload"):
        ...
        return (
            future is None
            or future.done()
            and time.time() > future.t_created + self.cfg["{}_interval".format(method)]
        )
    ...
    def _warn_uninferrable(self, symbol, item, to_push):
        ...
        self.xs.log2(
            "{} {} encountered uninferrable item: {}\n\n"
            "asks[:10] {}\n\nbids[:10] {}\n\nto_push: {}\n".format(
                self.name, symbol, item, asks, bids, to_push
            )
        )
    ...
    async def _init_orderbooks(self, cnx):
        ...
        for s in self.xs.sh.subscriptions:
            ...
            if s.channel != self.channel or s.cnx != cnx:
                continue
