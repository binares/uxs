...
class southxchange(ExchangeSocket):
    ...
    url_components = {"ws": "wss://www.southxchange.com/api/v2/connect"}
    channel_defaults = {
    ...
    def on_orderbook_snapshot(self, r):
        ...
        for item in r["v"]:
            d = {
                ...
                "bids": sorted(
                    [self.parse_ob_item(x) for x in item["b"] if x["b"]], reverse=True
                ),
                "asks": sorted(
                    [self.parse_ob_item(x) for x in item["b"] if not x["b"]]
                ),
                "timestamp": None,
                "datetime": None,
                "nonce": None,
    ...
    async def fetch_order_book(self, symbol, limit=None, params={}):
        if self.is_active():
            ...
            return await self.send(
                {"_": "fetch_order_book", "symbol": symbol, "limit": None}, True, id=id
            )
        else:
