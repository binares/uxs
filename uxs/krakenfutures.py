...
class krakenfutures(ExchangeSocket):
    ...
    def parse_ticker(self, r):
        ...
        return self.api.ticker_entry(
            symbol=symbol, **self.api.lazy_parse(r, keys, map), info=r
        )
    ...
    def on_orderbook(self, r):
        ...
        if is_snapshot:
            ob.update(
                self.api.parse_order_book(
                    r, ob["symbol"], price_key="price", amount_key="qty"
                )
            )
    ...
    def on_trade(self, r):
        ...
        self.update_trades([{"symbol": symbol, "trades": parsed}], enable_sub=True)
    ...
    def parse_trade(self, t):
        ...
        apply = {
            ...
            "takerOrMaker": lambda x: (
                "maker" if x.lower().startswith("maker") else "taker"
            )
            if x
            else None,
        }
    ...
    def on_order(self, r):
        ...
        if is_snapshot:
            ...
            _order = dict(r["order"], is_cancel=r["is_cancel"], reason=r["reason"])
            orders = [_order]
    ...
    def parse_order(self, o):
        ...
        remaining = (
            o["qty"] if not o.get("is_cancel") else 0
        )  # qty does indeed show remaining
    ...
    async def sign(self, out):
        ...
        if (
            not cache["original"]
            or cnx is None
            or cnx.connected_ts != cache["connected_ts"]
        ):
            R = await self.tp.send(
                {"_": "fetch_challenge"}, wait="default", id="fetch_challenge"
            )
            """{  
                "event":"challenge",
