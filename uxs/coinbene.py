...
class coinbene(ExchangeSocket):
    ...
    channel_ids = {
        ...
        "account": [
            "spot/userEvent",
            "margin/userEvent",
        ],  # spot/userEvent receives empty messages for some reason
    ...
    def on_ticker(self, r):
        ...
        tickers = [
            self.parse_ticker(x) for x in r["data"] if x["s"] in self.api.markets_by_id
        ]
    ...
    def parse_ticker(self, r):
        ...
        return self.api.ticker_entry(**self.api.lazy_parse(r, [], map, apply), info=r)
    ...
    def on_ohlcv(self, r):
        ...
        self.update_ohlcv(
            [{"symbol": symbol, "timeframe": timeframe, "ohlcv": ohlcvs}],
            enable_sub=True,
        )
    ...
    def sign(self, out={}):
        ...
        expires = (
            self.api.iso8601(expires_ts).split(".")[0] + "Z"
        )  # 2019-07-04T02:19:08Z
        signStr = expires + "GET" + "/login"
        signature = self.api.hmac(
            self.api.encode(signStr), self.api.encode(self.secret)
        )
        return {
            "op": "login",
