...
class kraken(ExchangeSocket):
    ...
    def check_errors(self, r):
        ...
        if (
            r.get("status") != "error" or "errorMessage" not in r
        ):  # or'event' not in r :
            self.notify_unknown(r)
            return
    ...
    def on_ticker(self, r):
        ...
        ticker = r[1]
        symbol_id = r[3]
        self.update_tickers([self.parse_ticker(ticker, symbol_id)], enable_sub=True)
    ...
    def on_orderbook(self, r):
        ...
        ob["datetime"] = (
            self.api.iso8601(timestamp) if timestamp is not None else timestamp
        )
    ...
    def on_order(self, r):
        ...
        for o_dict in orders:
            ...
            if id not in self.orders:
                ...
                self.update_order_from_dict(
                    o_parsed, enable_sub=True, drop={None: True}
                )
    ...
    def encode(self, req, sub=None):
        ...
        if sub is not None:
            ...
            if channel == "ohlcv":
                out["subscription"]["interval"] = int(
                    self.api.timeframes[p["timeframe"]]
                )
    ...
    async def sign(self, out):
        ...
        token = await call_via_loop(
            self._fetch_and_cache_auth_token, (5,), loop=self.loop, module="asyncio"
        )
    ...
    async def create_connection_url(self, url_factory):
        ...
        auth = (
            self.apiKey
        )  # and not self.is_subscribed_to({'_': 'account'}, active=True)
    ...
    def _start_wsToken_fetcher(self):
        ...
        t = AsyncTicker(
            self._fetch_and_cache_auth_token,
            60,
            keepalive={
                "attempts": True,
                "pause": 75,
    ...
    async def _fetch_and_cache_auth_token(self, renew_before=185):
        ...
        if (
            not self._cached_wstoken["token"]
            or time.time() > self._cached_wstoken["expires"] - renew_before
        ):
            ...
            self._cached_wstoken.update({"token": token, "expires": now + expires})

        return self._cached_wstoken["token"]

    async def ping(self):
        await self._socket.send(json.dumps({"event": "ping", "reqid": ctime_ms()}))

    def safe_currency_code(self, currency_id, currency=None):
        cy = next(
            (
                x["code"]
                for x in self.api.currencies.values()
                if x["info"].get("altname") == currency_id
            ),
            None,
        )
        if cy is None:
    ...
    def symbol_by_id(self, symbol_id):
        symbol = next(
            (
                x["symbol"]
                for x in self.api.markets.values()
                if x["info"].get("wsname") == symbol_id
            ),
            None,
        )
        if symbol is None:
