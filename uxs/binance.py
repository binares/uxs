...
class binance(ExchangeSocket):
    ...
    def on_all_tickers(self, r):
        ...
        for x in r:
            try:
                entries.append(self.on_ticker(x, send=False))
            except KeyError as e:
                reload_markets = True

        if (
            reload_markets
            and time.time() - getattr(self, "_markets_last_reloaded", 0) > 60
        ):
    ...
    def on_limited_ob_update(self, R):
        ...
        raise NotImplementedError(
            "{} - limited ob not supported yet".format(self.exchange)
        )
    ...
    def on_ohlcv(self, r):
        ...
        self.update_ohlcv(
            [{"symbol": symbol, "timeframe": tf, "ohlcv": [e]}], enable_sub=True
        )
    ...
    def on_order(self, r):
        ...
        if o is None:
            ...
            self.update_order(
                d["id"], d["remaining"], d["filled"], d["payout"], params=extra
            )
    ...
    async def fetch_listen_key(self):
        ...
        fapiMethod = (
            ...
            if hasattr(self.api, "fapiPrivatePostListenKey")
        ...
        r = await call_via_loop(fetch_key, loop=self.loop, module="asyncio")

        listen_key = r["listenKey"]

        r2 = await call_via_loop(
            self.prolong_listen_key_expiry,
            args=(listen_key,),
            loop=self.loop,
            module="asyncio",
        )
    ...
    async def prolong_listen_key_expiry(self, listen_key):
        ...
        fapiMethod = (
            ...
            if hasattr(self.api, "fapiPrivatePutListenKey")
        ...
        r = await call_via_loop(prolong_key, loop=self.loop, module="asyncio")
    ...
    def start_listen_key_prolonging(self, listen_key):
        ...
        ticker = AsyncTicker(
            ...
            args=(listen_key,),
            lock="next",
            keepalive={"attempts": True, "pause": 75, "exit_on": inactive},
        ...
        call_via_loop(prolong_and_cancel_on_inactive, loop=self.loop, module="asyncio")
