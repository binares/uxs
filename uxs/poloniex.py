...
CURRENCY_PAIR_IDS_PATH = os.path.join(
    os.path.dirname(__file__), "_data", "poloniex_currency_pair_ids.txt"
)
CURRENCY_IDS_PATH = os.path.join(
    os.path.dirname(__file__), "_data", "poloniex_currency_ids.txt"
)
...
read_currencies()
currency_pairs_ccxt = {
    id: "/".join(symbol.split("_")[::-1]) for id, symbol in currency_pairs.items()
}
...
class poloniex(ExchangeSocket):
    ...
    def on_ticker(self, r):
        ...
        for pair_data in r[2:]:
            ...
            vwap = (
                quote_cy_vol_24 / base_cy_vol_24 if base_cy_vol_24 else last_trade_price
            )
        ...
        for item in data:
            if item[0] == "i":
                ...
                new_obs["bids"] = [
                    ...
                    for price, qty in ob["orderBook"][1].items()
                ]
                new_obs["asks"] = [
                    [float(price), float(qty)]
                    for price, qty in ob["orderBook"][0].items()
                ]
                methods["create"] = True
            elif item[0] == "o":
    ...
    def on_new_order(self, item):
        ...
        if dto.tzinfo is not None:
            logger2.error(
                "POLONIEX HAS CHANGED THEIR DATE FORMAT: {}, {}".format(tstr, dto)
            )
    ...
    def on_fill(self, item):
        ...
        if dto.tzinfo is not None:
            logger2.error(
                "POLONIEX HAS CHANGED THEIR DATE FORMAT: {}, {}".format(item[8], dto)
            )
        ts = timestamp_ms(dto.replace(tzinfo=None))
        self.add_fill(
            id=tid,
            ...
            "fills": [
                {
                    "id": str(
                        t["info"]["tradeID"]
                    ),  # websocket only sends fill's 'tradeID', not 'globalTradeID'
                    "symbol": t["symbol"],
                    "side": t["side"],
                    "price": t["price"],
                    "amount": t["amount"],
                    ...
                    "fee_rate": round(
                        1
                        - float(t["info"]["takerAdjustment"])
                        / float(t["info"]["amount" if t["side"] == "buy" else "total"]),
                        5,
                    )
                    if "takerAdjustment" in t["info"]
                    else round(float(r["info"]["fee"]), 5),
                    "timestamp": t["timestamp"],
                    "datetime": t["datetime"],
                    "cost": t["cost"],
                    "order": r["id"],
                    "payout": float(t["info"]["takerAdjustment"])
                    if "takerAdjustment" in t["info"]
                    else None,
                }
    ...
    def id_to_cy(self, id):
        try:
            return next(
                x for x, y in self.api.currencies.items() if y["info"].get("id") == id
            )
    ...
    def id_to_symbol(self, id):
        try:
            return next(
                x for x, y in self.api.markets.items() if y["info"].get("id") == id
            )
        except (StopIteration, AttributeError):
