...
class kucoin(ExchangeSocket):
    ...
    def on_orderbook_update(self, r):
        ...
        for cur_nonce in range(start, end + 1):
            ...
            for side in ["bids", "asks"]:
                try:
                    changes = d["changes"][side]
                except KeyError:
                    continue
                update[side] = [
                    [float(price), float(qty)]
                    for price, qty, nonce in changes
                    if nonce == str_nonce
                ]
    ...
    async def create_connection_url(self):
        ...
        if auth:
            ...
            signature = sign(
                self.secret, str_to_sign, "sha256", hexdigest=False, base64=True
            )
            headers = {
    ...
    def encode(self, rq, sub=None):
        ...
        uid = None
        try:
            uid = self.get_subscription(rq).uid
        except ValueError:
            pass
