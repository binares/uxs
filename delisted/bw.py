...
class bw(ExchangeSocket):
    ...
    def parse_ob_update(self, u):
        ...
        return {"symbol": symbol, which: [[float(u[5]), float(u[6])]], other: []}
    ...
    def parse_ticker(self, t):
        ...
        return self.api.ticker_entry(
            ...
            quoteVolume=t[9],
            open=open,
            info=t,  # the volume seems to be of this day (not 24h)
        )
    ...
    def on_trade(self, r):
        ...
        self.update_trades(
            [{"symbol": trades[0]["symbol"], "trades": trades}], enable_sub=True
        )
    ...
    def encode(self, rq, sub=None):
        ...
        if "<symbol>" in dataType:
            dataType = dataType.replace(
                "<symbol>", self.convert_symbol(params["symbol"], 1)
            )
        if "<timeframe>" in dataType:
            dataType = dataType.replace(
                "<timeframe>", self.convert_timeframe(params["timeframe"], 1)
            )
        if "<name>" in dataType:
