...
class bittrex(ExchangeSocket):
    ...
    async def handle(self, R):
        ...
        # {'R': {'Success': True, 'ErrorCode': None}, 'I': '3'}
            ...
            if (
                isinstance(data["R"], list)
                and not all(x.get("Success") for x in data["R"])
                or isinstance(data["R"], dict)
                and not data["R"].get("Success")
            ):
                self.log_error(data)
    ...
    def parse_ticker(self, ticker):
        ...
        map = {"bid": "bidRate", "ask": "askRate", "last": "lastTradeRate"}
        return self.ticker_entry(
            symbol=symbol, **self.lazy_parse(ticker, [], map), info=ticker
        )
    ...
    def on_orderbook_delta(self, r):
        ...
        ob = dict(
            self.api.parse_order_book(
                r, symbol, None, "bidDeltas", "askDeltas", "rate", "quantity"
            ),
    ...
    def parse_order(self, order):
        ...
        clientOrderId = self.safe_string_2(
            order, "clientOrderId", "clientConditionalOrderId"
        )
    ...
    def on_balance(self, r):
        ...
        self.update_balances(
            [{"cy": cy, "free": free, "total": total, "info": d}], enable_sub=True
        )
    ...
    def sign(self, out=[]):
        ...
        signed_content = self.api.hmac(
            self.api.encode(content), self.api.encode(self.secret), hashlib.sha512
        )
