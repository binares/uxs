...
async def load_market_info(api, market, min_cost=None, tickers=None, cy_graph=None):
    ...
    prices = sorted([x[0] for x in ob['bids']] + [x[0] for x in ob['asks']] + [x['price'] for x in trades])
    amounts = sorted([x[1] for x in ob['bids']] + [x[1] for x in ob['asks']] + [x['amount'] for x in trades])
    _len = len(prices)
    ...
    limits = {
        ...
        #},
        'amount': {
            'min': amounts[int(_len/8)], # pick a smallish amount, but exclude the smallest which could be anomalous
        },
    }
    if hasattr(min_cost, '__iter__'):
        cost_in_currency, currency = min_cost
        symbol = market['quote'] + '/' + currency
        quote_price_in_currency = calc_price(symbol, {s: x['last'] for s,x in tickers.items()}, cy_graph)
        min_cost = cost_in_currency / quote_price_in_currency
        
        symbol2 = market['base'] + '/' + currency
        base_price_in_currency = calc_price(symbol2, {s: x['last'] for s,x in tickers.items()}, cy_graph)
...
async def load_info(api, info={}, *, min_cost=None, symbols=None, recalculate=False):
    ...
    for symbol, market in sorted(api.markets.items(), key=lambda x: x[0]):
        if symbol in info and not recalculate or symbols is not None and symbol not in symbols:
            continue
        try:
            market_info = await load_market_info(api, market, min_cost, tickers, api.load_cy_graph())
            if market_info:
                info[symbol] = market_info
        except Exception as e:
            logger2.error('{} - could not load {} precisions: {}'.format(api.id, symbol, repr(e)))
            logger.exception(e)
    return info
...
def main():
    exchange = sys.argv[1]
    apply = {'symbols': lambda x: x.split(','),
             'min_cost': _parse_cost}
    ...
    info = asyncio.get_event_loop().run_until_complete(
        load_info(api, previous_info, min_cost=min_cost, symbols=symbols, recalculate=recalculate))
    save_info(api, info)
